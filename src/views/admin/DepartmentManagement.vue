            ...
            <el-tabs v-model="activeTab">
              ...
              <el-tab-pane label="部门负责人" name="leader">
                <el-form label-width="120px">
                  ...
                  <el-button type="primary" @click="saveLeader">保存负责人</el-button>
                </el-form>
              </el-tab-pane></el-tabs>
          </el-card>
        </div>
...
export default {
          ...
          children: [
            {
              ...
              label: "杭州宏玥工程咨询有限公司",
              permissions:["p1","p2"],
              users:[{id:"1001",name:"张三",role:"咨询工程师"}],
              leader: null,
              children: [
                {
                  id: "3",
  ...
  methods: {
    ...
    handleNodeClick(node) {
      ...
      this.formData.type = node.type || "department";
      this.formData.level = node.level || "";
      this.formData.order = node.order || 0;

      // ---- 权限（与节点绑定）----
      this.checkedPermissions = node.permissions || [];

      // ---- 用户列表（与节点绑定）----
      this.userList = node.users ? JSON.parse(JSON.stringify(node.users)) : [];

      // ---- 负责人（与节点绑定）----
      this.currentLeader = node.leader || null;
    },
    ...
    saveInfo() {
      ...
      this.currentNode.permissions = this.checkedPermissions;
      this.$message.success("信息已保存（示例）");
    },
    saveLeader() {
      if (!this.currentNode) return;
      this.currentNode.leader = this.currentLeader;
      this.$message.success("负责人已保存（示例）");
    },
    /** 用户管理示例 */
    addUser() {
    ...
    removeUser(user) {
      ...
      this.currentNode.users=JSON.parse(JSON.stringify(this.userList))
    }
  }
};
</script>
